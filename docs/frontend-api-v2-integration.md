...
新增样式全部限定在 `.workflow-app` 命名空间。核心主题变量：
...
`WorkflowShell` 提供 `title`、`description`、`caseId`、`steps`、`currentStep`、`headerActions`、`children` 和 `theme` 属性。步骤、区块、通知和业务状态通过稳定的 `workflow-*` class、`data-step`、`data-state`、`data-current-step` 暴露；按钮与状态文案集中在 `frontend/lib/api-v2/copy.ts`。

Fixture 成功流使用 `data-theme="test"` 覆盖主题变量，证明换主题不需要修改 Gateway、差量构建或工作流状态机。后续甲方适配应优先替换：

1. `WorkflowShell` 页面外壳与导航容器。
2. `.workflow-app` 主题变量和布局令牌。
3. `copy.ts` 中的术语与状态文案。
4. 甲方患者、医生上下文到 `case_id`、`consultant_id`、`reviewer_id` 的启动参数映射。
...
- 交互控件最小高度 42px；窄屏表单、上传和审批布局折叠为单列。
...
当前边界：尚未获得甲方可编辑前端源码、真实后端测试环境、真实 Token 获取方式、患者上下文和权限指令。因此本轮不能证明甲方真实鉴权、病例归属、上传限制、轮询延迟或 PDF 网关行为。
