...
export function DraftApproval({ draft, value, onChange, busy, onContinue }: DraftApprovalProps) {
  ...
  return (
    <div className="workflow-stack">
      ...
      {excludedDecisions.length ? <details className="workflow-report-sections"><summary>规则排除记录（{excludedDecisions.length}）</summary><ul>{excludedDecisions.map((item) => <li key={`${item.rule_id}-${item.sku_id ?? "case"}`}>{item.message}</li>)}</ul></details> : null}
      {draft.structured_system_findings.length ? (
        <details className="workflow-report-sections">
          ...
          <ul>{draft.structured_system_findings.map((finding) => {
            ...
            return <li key={finding.system_id}><strong>{finding.system_name}：</strong>{!uncovered ? "已由当前方案覆盖" : reason === "safety_excluded" ? "候选产品未通过安全校验" : reason === "no_approved_mapping" ? "暂无批准的产品映射" : "当前证据未达到推荐条件"}</li>;
          })}</ul>
        </details>
      ) : null}
      {draft.manual_review_required ? <WorkflowNotice tone="warning">该方案要求人工复核，进入最终报告前请逐项确认推荐、剂量和安全提示。</WorkflowNotice> : null}
        ...
        {draft.recommended_skus.map((item) => {
          ...
          return (
            <article className="workflow-recommendation" key={item.sku_id} data-state={excluded ? "excluded" : "included"}>
              <div className="workflow-recommendation__header"><div><h3>{item.display_name}</h3><p>{item.reason}</p>{reviewDecisions.length ? <span className="workflow-status-badge">需要医生确认</span> : null}{warningDecisions.length ? <span className="workflow-status-badge">注意事项</span> : null}</div><label className="workflow-check workflow-inclusion-control"><input type="checkbox" checked={!excluded} disabled={busy || approved} onChange={(event) => onChange({ ...value, excludedSkuIds: event.target.checked ? value.excludedSkuIds.filter((id) => id !== item.sku_id) : [...value.excludedSkuIds, item.sku_id] })} /><span>纳入报告</span></label></div>
              <div className="workflow-form-grid"><label className="workflow-field"><span>批准剂量</span><select value={selectedOptionId} disabled={busy || approved || excluded} onChange={(event) => onChange({ ...value, dosageSelections: { ...value.dosageSelections, [item.sku_id]: event.target.value } })}>{item.dosage_options.map((option) => <option key={option.option_id} value={option.option_id}>{option.label}：{option.display_text}</option>)}</select></label>{changed && !excluded ? <label className="workflow-field"><span>调整说明（必填）</span><input value={value.dosageNotes[item.sku_id] ?? ""} disabled={busy || approved} onChange={(event) => onChange({ ...value, dosageNotes: { ...value.dosageNotes, [item.sku_id]: event.target.value } })} /></label> : null}</div>
              <div className="workflow-recommendation__details">{item.evidence_details.length ? <p><strong>证据：</strong>{item.evidence_details.join("；")}</p> : null}{item.dosage_match_reasons.length ? <p><strong>剂量依据：</strong>{item.dosage_match_reasons.join("；")}</p> : null}{item.warnings.length ? <p className="workflow-danger-text"><strong>提示：</strong>{item.warnings.join("；")}</p> : null}{item.current_supplement_overlap_notice ? <p className="workflow-danger-text"><strong>当前补充剂重叠：</strong>{item.current_supplement_overlap_notice}</p> : null}</div>
            </article>
          );
        })}
